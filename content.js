...
function extractDueDate(text) {
    ...
    dueDatePattern = /(?:\b(mon|tue|wed|thu|fri|sat|sun)\b,?\s*)?(?:@?\s*(\d{1,2}:\d{2}\s*(?:am|pm)))?\s*(?:on\s*)?(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[a-z]*\.?\s*\d{1,2}(?:st|nd|rd|th)?,?\s*(?:\b(mon|tue|wed|thu|fri|sat|sun)\b)?/i;
...
function scrapePageForAssignmentInfo() {
    ...
    // Log the results or send them to background.js
    ...
    if (assignmentName && dueDate) {
        chrome.runtime.sendMessage({ 
            action: 'processData', 
            data: { assignmentName: assignmentName, dueDate: dueDate }
        });
    }
}
